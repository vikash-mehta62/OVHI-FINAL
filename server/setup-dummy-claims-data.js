...
async function setupDummyClaimsData() {
  ...
  try {
    ...
    const sqlFilePath = path.join(__dirname, 'sql', 'add_dummy_claims_data_fixed.sql');
    const sqlContent = await fs.readFile(sqlFilePath, 'utf8');
    console.log('✅ SQL file loaded successfully');

    // Execute the SQL commands in chunks
      ...
      if (statement.length > 0) {
        try {
          ...
          if (statement.toLowerCase().includes('create table')) {
            console.log(`✅ Created table: ${statement.match(/CREATE TABLE.*?(\w+)/i)?.[1] || 'unknown'}`);
          } else if (statement.toLowerCase().includes('insert')) {
            console.log(`✅ Inserted data into: ${statement.match(/INSERT.*?INTO\s+(\w+)/i)?.[1] || 'unknown'}`);
          }
        } catch (error) {
